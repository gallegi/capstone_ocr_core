...
if __name__ == '__main__':
    ...
    fonts_source_zip_filepath = tools.download_and_verify(
    ...
    added = []
    with zipfile.ZipFile(file='data/fonts.zip', mode='w') as zfile:
    ...
    shutil.rmtree('data/backgrounds', ignore_errors=True)
    os.makedirs('data/backgrounds')
    ...
    paths_with_text = [
        filepath for filepath in tqdm.tqdm(filtered_paths) if len(
            detector.detect(
                images=[tools.read_and_fit(filepath, width=640, height=640)])[0]) > 0
    ]
    ...
    for filepath in removed_paths:
        os.remove(filepath)
    with open('data/backgrounds/urls.txt', 'w') as f:
        f.write('\n'.join(filtered_urls))
    with zipfile.ZipFile(file='data/backgrounds.zip', mode='w') as zfile:
        for filepath in tqdm.tqdm(filtered_paths.tolist() + ['data/backgrounds/urls.txt'],
