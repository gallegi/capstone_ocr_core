...
def get_text_generator(alphabet=None, lowercase=False, max_string_length=None):
    ...
    word_generator = None
    text_generator = None
    if os.path.isfile(word_gen_model_path):
        word_generator = MarkovWordGenerator(model=word_gen_model_path)
    if os.path.isfile(text_gen_model_path):
        text_generator = MarkovTextGenerator(model=text_gen_model_path)

    gen = essential_generators.DocumentGenerator(word_generator=word_generator, text_generator=text_generator)
    while True:
